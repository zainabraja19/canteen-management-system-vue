...
export default {
  ...
  async fetchData(context, payload) {
    try {
      const res = await fetch(
        `${baseUrl}/employee/${payload.empId}/${payload.type}`,
        { credentials: 'include' }
      );
      ...
      if (response.data[payload.type]) {
        ...
        const blob = new Blob([buffer], {
          type: response.data[payload.type].mimeType,
        });
        const url = window.URL.createObjectURL(blob);

        if (payload.type === 'resume') {
          context.commit('setResume', {
            resume: { url, name: response.data['resume'].originalName },
          });
        } else {
        ...
        context.commit(
          payload.type === 'resume' ? 'setResume' : 'setProfilePicture',
          { [payload.type]: null }
        );
      }
    } catch (err) {
  ...
  async cartCount(context, payload) {
    try {
      const res = await fetch(
        `${baseUrl}/employee/${payload.empId}/cart/cartCount`,
        { credentials: 'include' }
      );
      const response = await res.json();
  ...
  async fetchCart(context, payload) {
    try {
      const res = await fetch(`${baseUrl}/employee/${payload.empId}/cart`, {
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async fetchEmployeeOrders(context, payload) {
    try {
      const res = await fetch(
        `${baseUrl}/employee/${payload.empId}/order?page=${payload.page}&filter=${payload.filter}`,
        {
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  async cancelOrder(context, payload) {
    try {
      const res = await fetch(
        `${baseUrl}/employee/${payload.empId}/order/${payload.orderId}`,
        {
          method: 'PATCH',
          credentials: 'include',
          headers: {
  ...
  async generateInvoice(context, payload) {
    ...
    try {
      const res = await fetch(
        `${baseUrl}/employee/${payload.empId}/order/${payload.orderId}/generateInvoice`,
        {
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json',
  ...
  async verifyPayment(context, payload) {
    try {
      const res = await fetch(
        `http://localhost:3000/employee/A624/order/verifyPayment`,
        {
          method: 'POST',
          credentials: 'include',
          headers: {
