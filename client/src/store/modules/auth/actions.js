...
export default {
  ...
  async auth(context, payload) {
    ...
    try {
      ...
      const expirationDuration =
        new Date(response.data.expiresIn).getTime() - Date.now();

      //   document.cookie = `user=${new Date(
      //     response.data.expiresIn
      //   ).getTime()}; expires=${new Date(
      //     response.data.expiresIn
      //   ).toGMTString()};`;
  ...
  async autoLogin(context) {
    try {
      const res = await fetch(`${baseUrl}/auth/autoLogin`, {
        credentials: 'include',
      });
      ...
      if (response.data) {
        ...
        const expirationDuration =
          new Date(+expiresAttribute).getTime() - new Date().getTime();
  ...
  async autoLogout(context) {
    ...
    await context.commit('setAutoLogout');
  },
};
