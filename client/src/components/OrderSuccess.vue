<template>
    ...
    <div class="d-flex justify-content-center align-items-center" style="min-height: 80vh">
        <div class="card p-4 col-xl-5 col-lg-6 col-md-9 col-sm-11 col-12 shadow bg-body rounded text-center ">
            ...
            <p class="mt-3 px-5">Thank you for placing order. You can collect it from Canteen in <strong>approx 15
                    minutes</strong></p>
            <div class="btn-group px-4 my-2 row row-cols-1 row-cols-sm-2 g-2 g-sm-4" role="group"
                aria-label="Basic example">
                <div class="col">
                    <router-link to="/" type="button" class="go-back btn rounded w-100"><i class="bi bi-box-arrow-left me-2
                        "></i>Return
                        to menu</router-link>
                </div>
                <!-- </router-link> -->
                <div class="col-12">
                    <button type="button" class="btn print-receipt rounded w-100" @click="generateInvoice"><i
                            class="bi bi-file-earmark-arrow-down me-2"></i>Print
                        Receipt</button>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        async generateInvoice() {
            console.log(this.$store.getters['employee/currentOrderId']);

            const res = await this.$store.dispatch('employee/generateInvoice', {
                empId: await this.$store.getters['auth/user'].empId,
                orderId: await this.$store.getters['employee/currentOrderId']
            })
